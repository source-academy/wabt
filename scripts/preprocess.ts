 ...
 */
async function removeSelfImport(
  filePath: string,
  encoding: BufferEncoding = 'utf-8',
): Promise<void> {
  ...
  if (fileName.endsWith('.ts')) {
    // Technically, you can't do import x from 'file.ts', but whatever
    potentialImportNames = [
      fileName,
      fileName.slice(0, fileName.indexOf('.ts')),
    ];
  } else if (fileName.endsWith('.js')) {
    potentialImportNames = [
      fileName,
      fileName.slice(0, fileName.indexOf('.js')),
    ];
  }
  ...
  return outputFile(filePath, fileString.replace(selfImportRegex, ''));
}
...
export async function main() {
  ...
  const outputFiles = inputFiles.map((file) => file.replace(/.cpp.ts$/, '.ts')
    .replace(/.cpp.js$/, '.js'));

  await Promise.all(
    inputFiles.map((x, i) => preprocess(inputFiles[i], outputFiles[i])),
  );
  await Promise.all(outputFiles.map((file) => removeSelfImport(file)));
  await Promise.all(outputFiles.map((file) => fixEslint(file)));
}
